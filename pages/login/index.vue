<template>
	<view class="container login-page">
		...
		<view class="content">
			...
			<view class="form-area">
				...
				<view class="input-group">
					...
					<input class="input" type="number" placeholder="请输入手机号" placeholder-class="placeholder-text"
						v-model="phone" />
				</view>
				...
				<view class="input-group">
					...
					<input class="input" :type="showPassword ? 'text' : 'password'" placeholder="请输入密码"
						placeholder-class="placeholder-text" v-model="password" />
					<!-- <view class="icon eye-icon" @click="togglePassword"></view> -->
					<image class="icon eye-icon" :src="showPassword ? '/static/eye.png' : '/static/eyes.png'"
						@click="togglePassword"></image>
				</view>

				<!-- 登录按钮 -->
				<button class="btn-login" @click="handleLogin">登录</button>
				<button class="btn-login" @getphonenumber="getPhone" open-type="getPhoneNumber">微信登录</button>
		...
		<view class="popup-mask" v-if="showPopup" @click="closePopup" @touchmove.stop.prevent></view>
		...
		<view class="popup-box" v-if="showPopup" @touchmove.stop.prevent>
			...
			<view class="node-list">
				<view class="node-item" v-for="(item, index) in nodeList" :key="index" @click="selectNode(index)">
					<text class="node-name">{{ item.name }}</text>

					<!-- 选中状态显示绿色背景打钩，未选中显示灰色圆圈 -->
					<view class="radio-circle" :class="{ checked: activeNodeIndex === index }">
